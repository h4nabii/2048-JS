...
export default class GameCore {
  ...
  private getRandomAvailable(): [number, number] {
    ...
    const availableCells = this.grid.availableCells;
  ...
  randomAdd(): CellInfo {
    if (this.grid.full) return null;
    const value = GameCore.getRandomNumber();
    const [row, col] = this.getRandomAvailable();
  ...
  private *moveTraversal(dir: Direction): Generator<{
    ...
    const mergePositionList = new Set<string>();

    const info = this.getLoopInfo(dir);
    for (
      let row = info.rowLoop.start;
      row !== info.rowLoop.end;
      row += info.rowLoop.step
    ) {
      for (
        let col = info.colLoop.start;
        col !== info.colLoop.end;
        col += info.colLoop.step
      ) {
        if (!this.grid.isAvailable(row, col)) {
          ...
          if (
            this.grid.get(nextRow, nextCol) === null ||
            this.grid.get(nextRow, nextCol)?.valueOf() !==
              this.grid.get(row, col)?.valueOf() ||
  ...
  move(dir: Direction): MoveState {
    ...
    for (const { cur, next, merge } of this.moveTraversal(dir)) {
      this.grid.move(...cur, ...next);
      state.moveCount++;
      state.tileChanges.move.push({ origin: cur, target: next });

      if (merge) {
        const mergedValue = this.grid.get(...next)?.valueOf() * 2;
  ...
  get over() {
    ...
    for (let i = 1; i <= this.height; i++) {
      for (let j = 1; j <= this.width; j++) {
        if (
          this.grid.get(i, j).valueOf() ===
            this.grid.get(i + 1, j)?.valueOf() ||
