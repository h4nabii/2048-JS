...
export default class DisplayManager {
  ...
  mergeTile(row: number, col: number) {
    const tiles = document.querySelectorAll(
      `.${this.AnimationStyles.position(row, col)}`
    );
    const tile = tiles[0],
      oldTile = tiles[1];
    const number = +tile.textContent;
    oldTile.remove();
  ...
  addScore(score: number) {
    this.doms.score.textContent = (
      +this.doms.score.textContent + score
    ).toString();
  }
  ...
  resume() {
    const maskClass = this.doms.pauseMask.classList;
